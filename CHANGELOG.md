...
- added: render muster `MCPServer` CRs from the `mcpServers` list (`muster.enabled`). Verified a
  semantically identical drop-in for the hand-written files it replaces, across all four auth modes.
- added: render agentgateway configuration (`AgentgatewayBackend` + `AgentgatewayPolicy`)
